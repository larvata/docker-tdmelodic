...
RUN apk add --no-cache libstdc++ libgcc bash curl openssl wget sudo autoconf automake build-base &&\
...
RUN rm -rf /workspace
RUN rm -rf /tmp
RUN apk del curl openssl wget sudo autoconf automake build-base
